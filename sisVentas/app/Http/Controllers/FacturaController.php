     ...
     public function create()
    {
                ...
                public function show ()
                {
                      ...
                      $id=$_GET['id'];//Aqui recibo de la la vista show de devolucion, enviada por una funcion jquery al controlador a la ruta Route::get('pfd/notacredito/{iddevolucion}','NotaCreditoController@show'); para poder imprimir la nota de credito o devolucion seleccionada por el susuario
                        ...
                        ->select('v.idventa','v.fecha_hora','p.nombre','v.tipo_comprobante','v.serie_comprobante','v.num_comprobante','v.impuesto','v.estado','v.total_venta','v.estatus_venta','v.estatus_venta','p.tipo_documento AS tipo_documento','p.num_documento AS num_documento','p.telefono AS telefono_cliente','p.nombre AS cliente','p.direccion AS direccion_cliente','p.codigo AS codigo_cliente','vend.nombre AS vendedor','vend.num_documento AS cedula_vendedor','vend.tipo_documento AS documento_vendedor','vend.telefono AS telefono','td.nombre AS tienda','td.codigo AS codigo_tienda','td.direccion AS tienda_direccion','td.telefono AS tienda_tlf','td.rif AS tienda_rif')
                        ->where('v.idventa','=',$id)
                        ...
                        ->select('a.nombre as articulo','a.codigo as codigo','a.unidad_medida as unidad_medida','a.descripcion as descripcion','d.cantidad','d.descuento','d.precio_venta')
                        ->where('d.idventa','=',$id)
                        ->get();
